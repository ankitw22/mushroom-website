...
export default function AppPage() {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--cream)' }}>
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 13, color: 'rgba(10,10,10,0.35)' }}>Loading…</span>
      </div>
    );
  }
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--cream)' }}>
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontFamily: "'Poppins',sans-serif", fontSize: 18, color: '#0a0a0a', marginBottom: 16 }}>App not found.</p>
          <Link href="/" style={{ color: '#068F57', fontFamily: "'Poppins',sans-serif", textDecoration: 'underline' }}>← Back to home</Link>
        </div>
      </div>
    );
  }
  ...
  const uniqueCombos = allCombinations.filter(
    (c, i, arr) => arr.findIndex(x => x.description === c.description) === i
  ).slice(0, 12);
      ...
      <div style={{ margin: '10px 10px 0' }}>
        <div style={{
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 48px 52px' }}>
            ...
            {/* Split layout: left text + right chat mockup */}
          ...
          <div style={{ display: 'flex', gap: 4, background: 'rgba(10,10,10,0.06)', borderRadius: 10, padding: 4 }}>
            {(['actions', 'triggers'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} style={{
                ...
                fontFamily: "'Poppins',sans-serif", fontSize: 13, fontWeight: activeTab === tab ? 700 : 400,
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
            {connectedApps.map(connApp => (
              <Link key={connApp.appslugname} href={`/app/${connApp.appslugname}`}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', background: '#fff', border: '1.5px solid rgba(10,10,10,0.07)', borderRadius: 12, textDecoration: 'none', transition: 'border-color 0.2s' }}
              >
